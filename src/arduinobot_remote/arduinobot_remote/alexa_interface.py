...
threading.Thread(target=lambda: rclpy.spin(Node("alexa_interface"))).start()
...
class PickupIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_text = "ok I am moving to the object"

        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Picking up the object", speech_text)).set_should_end_session(
            True)
...
class SleepIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speech_text = "ok I am going to sleep"

        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Going to sleep", speech_text)).set_should_end_session(
            True)
...
class WakeupIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speech_text = "ok I am waking up"

        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Waking up", speech_text)).set_should_end_session(
            True)
...
skill_builder.add_request_handler(PickupIntentHandler())
